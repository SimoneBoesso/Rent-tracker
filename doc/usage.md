...
`shap_values` (sorted by \|contribution\|) and `shap_base_value` explain the prediction via TreeSHAP on the preprocessed features (`base + Σ shap ≈ predicted`; HGB treats ordinal-encoded cats as numeric so TreeSHAP stays additive).  
